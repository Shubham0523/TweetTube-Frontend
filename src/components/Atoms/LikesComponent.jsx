...
function LikesComponent({
  ...
  // Add useEffect to sync with props when they change
  useEffect(() => {
    setLike({ isLiked, totalLikes });
    setDislike({ isDisLiked, totalDisLikes });
  ...
  const handleToggleLike = async (status) => {
    ...
    setLike({ isLiked: localLike, totalLikes: localTotalLike });
    setDislike({ isDisLiked: localDislike, totalDisLikes: localTotalDisLike });
    ...
    try {
      const response = await dispatch(toggleLike({ qs, toggleLike: status }));
      
      // Update with server response
      if (response.payload) {
        const { isLiked, totalLikes, isDisLiked, totalDisLikes } = response.payload;
        setLike({ isLiked, totalLikes });
        setDislike({ isDisLiked, totalDisLikes });
      }
    } catch (error) {
